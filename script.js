...
const minimizedFullscreenButton = document.getElementById('minimized-graph-fullscreen')
...
const minimizedGraphContainer = document.getElementById('minimized-graph-container')
...
function checkToStates() {
  ...
  validToStates = assertSetContains(inputAutomata.states, values) && values.size !== 0
...
function checkTransition() {
  ...
  addTransitionElement(fromStateInput.value, symbolInput.value, toStatesInput.value)
  plot()
...
function onClose(e) {
  ...
  inputAutomata.removeTransition(fromStateInput.value, symbolInput.value, toStatesInput.value)
...
function removeInvalidTransitions() {
  ...
  for (const transition of transitions) {
    ...
    if (!assertSetContains(inputAutomata.states, getInput(toStatesInput.value))) {
      inputAutomata.removeTransition(fromStateInput.value, symbolInput.value, toStatesInput.value)
      transition.remove()
      continue
    }
...
function lockProperties() {
  ;[statesInput, alphabetInput, initialStatesInput, finalStatesInput].forEach((element) => {
